...
for mset in FILTERED_MULTISETS_8:
    counts = 0
    for r in range(13):
        counts |= (np.int64(mset.count(r)) << np.int64(4 * r)) # Pack counts into nibbles for each rank in a single int64
...
def _calculate_base_weight_8(mset_counts: int, hand_rank_counts: int, rem_rank_counts: int) -> tuple[int, int]:
    ...
    while temp_need_counts:
        ...
        weight *= COMB_LUT[(rem_rank_counts >> shift) & np.int64(0xF)][(need_counts >> shift) & np.int64(0xF)]
        temp_need_counts &= (~(np.int64(0xF) << np.int64(shift)))

    return weight, need_counts


@njit(Tuple((int64, int64))(int64, int64, int64, int64, int64[:], int64), cache=True, fastmath=True)
def _calculate_flush_weight_8(hand: int, total_needed: int, need_counts: int, available_set: int, flush_lut: np.ndarray, non_flush_score: int) -> tuple[int, int]:
    flush_weight = 0
    flush_treys_rank_sum = 0
    avail_counts_packed = 0
    for r in range(13):
        count = custom_bit_count((available_set >> (4 * r)) & np.int64(0xF))
        avail_counts_packed |= (count << (4 * r))
    ...
    needed_suit_mask = 0
    for r in range(13):
        if (need_counts >> (4 * r)) & np.int64(0xF):  # If we need cards of this rank
            needed_suit_mask |= (np.int64(1) << r)
    ...
    for suit_num in range(4):
        ...
        if (prepicked_suit_count + min(custom_bit_count(available_set & STENCILS[suit_num]), total_needed)) >= 5:
            ...
            has_flush_suit_mask = (available_set >> suit_num) & np.int64(0x1111111111111)
            temp_mask = needed_suit_mask
            while temp_mask:
                lsb = temp_mask & -temp_mask
                r = custom_bit_length(lsb) - 1
                shift = 4 * r
                needed = (need_counts >> shift) & np.int64(0xF)
                avail_r = (available_set >> shift) & np.int64(0xF)
                has_flush_suit = (has_flush_suit_mask >> shift) & np.int64(1)
                others_avail = custom_bit_count(avail_r) - has_flush_suit
                
                # Use precompiled combinations LUT. math.comb(n, k) -> 0 natively if k > n
                wnp = COMB_LUT[others_avail][needed]
                wp = COMB_LUT[others_avail][needed - 1] if has_flush_suit else 0

                if wp > 0:
                    candidate_mask |= (np.int64(1) << r) # We optionally physically can pick a flush suit card here
            ...
            while temp_fixed:
                ...
                r = custom_bit_length(lsb) - 1
                base_w *= (ways_no_pick >> (4 * r)) & np.int64(0xF)
                temp_fixed ^= lsb
            
            # 2. Submask iteration
            sub = candidate_mask
            cards_suit_needed = 5 - prepicked_suit_count
            ...
            while True:
                ...
                if custom_bit_count(flush_cards_mask) >= 5:
                    ...
                    while temp_cand:
                        ...
                        shift = 4 * r
                        if sub & lsb:
                            w *= (ways_pick >> shift) & np.int64(0xF)
                        else:
                            w *= (ways_no_pick >> shift) & np.int64(0xF)
...
@njit((float64)(int64, int64, int64, int64, int64, int64[:, :], int64[:]), cache=True, fastmath=True)
def _evaluate_dealer_loop(hand_bits, hand_rank_counts, available_set, rem_rank_counts, cards_needed, fast_iter_arr, flush_lut):
    total_ev = 0.0
    total_weight = 0
    ...
    for i in range(fast_iter_arr.shape[0]):
        ...
        weight, need_counts = _calculate_base_weight_8(mset_counts, hand_rank_counts, rem_rank_counts)
        if weight > 0:
            flush_weight, total_flush_score = _calculate_flush_weight_8(
                hand_bits, cards_needed, need_counts, available_set, flush_lut, non_flush_score
            )
