...
COMB_LUT = np.array([[math.comb(n, k) for k in range(6)] for n in range(5)], dtype=np.int64)

# change lut depending on mapping !!!!!!
PRECOMPUTED_EARLY_GAME_LUT_PATH = os.path.join(os.path.dirname(__file__), 'early_game_player_lut_p09.json')
...
# an example mapping from treys rank to a true rank value
# 
# intuitively, the difference between rank 1 and 2 should be far larger than that between rank 7461 and 7462.
# so the mapping should be concave and increasing. sqrt is a simple choice that meets these criteria.
#
# one can also use the cdf of treys rank under random rollout
...
def _calculate_base_weight(mset_counts: int, hand_rank_counts: int, rem_rank_counts: int, COMB_LUT) -> tuple[int, int]:
    weight = 1
    ...
    # cast it to 64-bit to prevent corruption on high cards.
...
@njit(float64(int64, int64, int64, int64, int64, int64[:, :], int64[:, :]), cache=True, fastmath=True, parallel=True)
def _evaluate_player_loop(hand_bits, hand_rank_counts, available_set, rem_rank_counts, cards_needed, COMB_LUT, fast_iter_arr):
    total_ev = 0.0
    total_weight = 0
    ...
    for i in prange(fast_iter_arr.shape[0]):
        ...
        if not (((((hand_rank_counts + rem_rank_counts) | CARRY_CHECKER) - mset_counts) & CARRY_CHECKER) != CARRY_CHECKER or
            ...
            flush_score = UTILITY_MAP[fast_iter_arr[i, 1]]
            non_flush_score = UTILITY_MAP[fast_iter_arr[i, 2]]
            weight, need_counts = _calculate_base_weight(mset_counts, hand_rank_counts, rem_rank_counts, COMB_LUT)
...
def _cached_evaluate_player(hand_bits, hand_rank_counts, available_set, rem_rank_counts, cards_needed):
...
def evaluate_player_deterministic(hand, hand_bits, hand_rank_counts, available_set, rem_rank_counts):
    ...
    if cards_needed == 0:
        return UTILITY_MAP[get_best_score(hand)]
    ...
    # Pass only integers into the cached loop
    return _cached_evaluate_player(
        hand_bits, 
        hand_rank_counts, 
        available_set, 
        rem_rank_counts, 
...
# change lut depending on mapping !!!!!!
PRECOMPUTED_EARLY_GAME_LUT_8_PATH = os.path.join(os.path.dirname(__file__), 'early_game_dealer_lut_p09.json') # Change to your actual filename
PRECOMPUTED_EARLY_GAME_LUT_8 = {}

if os.path.exists(PRECOMPUTED_EARLY_GAME_LUT_8_PATH):
    with open(PRECOMPUTED_EARLY_GAME_LUT_8_PATH, 'r') as f:
        # Note: If your keys are strings of (hand_bits, dead_bits), load them appropriately
        PRECOMPUTED_EARLY_GAME_LUT_8 = json.load(f)
...
# non_flush_score should be the original treys rank
@njit(Tuple((int64, float64))(int64, int64, int64, int64, int64[:], int64, int64[:, :]), cache=True, fastmath=True)
def _calculate_flush_weight_8(hand: int, total_needed: int, need_counts: int, available_set: int, flush_lut: np.ndarray, non_flush_score: int, COMB_LUT) -> tuple[int, float]:
    flush_weight = 0
    flush_treys_rank_sum = 0.0
            ...
            while True:
                ...
                if custom_bit_count(flush_cards_mask) >= 5:
                    ...
                    if w > 0:
                        ...
                        flush_treys_rank_sum += w * UTILITY_MAP[best_flush_branch_score]
...
def _evaluate_dealer_loop(hand_bits, hand_rank_counts, available_set, rem_rank_counts, cards_needed, fast_iter_arr, flush_lut, COMB_LUT):
    ...
    for i in prange(fast_iter_arr.shape[0]):
        ...
        if not (((((hand_rank_counts + rem_rank_counts) | CARRY_CHECKER) - mset_counts) & CARRY_CHECKER) != CARRY_CHECKER or
            ...
            non_flush_weight = weight - flush_weight
            total_ev += (non_flush_weight * UTILITY_MAP[non_flush_score]) + total_flush_score
            total_weight += weight
...
def evaluate_dealer_deterministic(hand, hand_bits, hand_rank_counts, available_set, rem_rank_counts):
    ...
    if lut_key in PRECOMPUTED_EARLY_GAME_LUT_8:
        return PRECOMPUTED_EARLY_GAME_LUT_8[lut_key]
